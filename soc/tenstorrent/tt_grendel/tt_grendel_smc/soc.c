...
void soc_early_init_hook(void)
{
	...
	if (DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(uart0))) {
		/* Enable UART0 */
		uart_ctrl.val = sys_read32(UART_WRAP0_UART_CTRL_REG_ADDR);
		uart_ctrl.f.uart_en = 1;
		uart_ctrl.f.uart_reset_n_n0_scan = 1;
		sys_write32(uart_ctrl.val, UART_WRAP0_UART_CTRL_REG_ADDR);
	}
	if (DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(uart1))) {
		/* Enable UART1 */
		uart_ctrl.val = sys_read32(UART_WRAP1_UART_CTRL_REG_ADDR);
		uart_ctrl.f.uart_en = 1;
		uart_ctrl.f.uart_reset_n_n0_scan = 1;
		sys_write32(uart_ctrl.val, UART_WRAP1_UART_CTRL_REG_ADDR);
	}

	if (DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(uart2))) {
		/* Enable UART2 */
		uart_ctrl.val = sys_read32(UART_WRAP2_UART_CTRL_REG_ADDR);
		uart_ctrl.f.uart_en = 1;
		uart_ctrl.f.uart_reset_n_n0_scan = 1;
		sys_write32(uart_ctrl.val, UART_WRAP2_UART_CTRL_REG_ADDR);
	}

	if (DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(uart3))) {
		/* Enable UART3 */
		uart_ctrl.val = sys_read32(UART_WRAP3_UART_CTRL_REG_ADDR);
		uart_ctrl.f.uart_en = 1;
		uart_ctrl.f.uart_reset_n_n0_scan = 1;
		sys_write32(uart_ctrl.val, UART_WRAP3_UART_CTRL_REG_ADDR);
	}
}
