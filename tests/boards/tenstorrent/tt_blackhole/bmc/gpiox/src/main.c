...
static void test_gpiox_common(int i)
{
	...
	for (int j = 0; j < NGPIOS; j++) {
		 ...
		 */
		zassert_ok(gpio_pin_get_config(dev, j, &flags),
			   "failed to get gpio config for port %d, pin %d", i, j);
	}
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio0)
{
	test_gpiox_common(0);
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio1)
{
	test_gpiox_common(1);
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio2)
{
	test_gpiox_common(2);
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio3)
{
	test_gpiox_common(3);
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio4)
{
	test_gpiox_common(4);
}

ZTEST(tt_blackhole_bmc_gpiox, test_gpio5)
{
	test_gpiox_common(5);
}

ZTEST_SUITE(tt_blackhole_bmc_gpiox, NULL, NULL, NULL, NULL, NULL);
