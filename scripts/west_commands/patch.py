...
with open(WEST_PATCH_SCHEMA_PATH, "r") as f:
...
class Patch(WestCommand):
    ...
    def do_add_parser(self, parser_adder):
        ...
        parser.add_argument(
            ...
            metavar="DIR",
            default=Path("WEST_MANIFEST_DIR") /
            WEST_PATCH_BASE)
        parser.add_argument(
            "-l",
            "--patch-yml",
        ...
        subparsers = parser.add_subparsers(
            dest="subcommand",
            metavar="<subcommand>",
            help="select a subcommand. If ommitted treat it as 'list'")
    ...
    def do_run(self, args, _):
        ...
        try:
            with open(args.patch_yml, "r") as f:
                yml = yaml.load(f, Loader=SafeLoader)
            pykwalify.core.Core(source_data=yml,
                                schema_data=patches_schema).validate()
