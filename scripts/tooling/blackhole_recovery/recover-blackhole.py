...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If set, automatically select the first available adapter (default: False)",
    )
...
def get_session(asic, adapter_id, select_first, temp_dir):
...
def main():
    ...
    with (
        ...
        for idx in range(len(BOARD_ID_MAP[args.board])):
            ...
            session = get_session(asic, args.adapter_id, args.select_first, temp_dir)
            session.open()
