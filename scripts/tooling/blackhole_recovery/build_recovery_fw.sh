...
build_recovery_fw() {
    ...
    srec_cat build/mcuboot/zephyr/zephyr.hex -intel build/dmc/zephyr/zephyr.signed.hex -intel -o $OUTPUT_DIR/$BOARD/dmc_fw.hex -intel
    ...
    ./scripts/tt_boot_fs.py mkfs --build-dir build --hex boards/tenstorrent/tt_blackhole/bootfs/$BOARD-bootfs.yaml build/smc_fw.hex
    ...
    echo $(git describe) > $OUTPUT_DIR/$BOARD/VERSION
    echo "NOTE: a patch was applied to this tag to support generating the board ID within the readonly section" >> $OUTPUT_DIR/$BOARD/VERSION
}
