...
static int pcie_nftw_callback(const char *fpath, const struct stat *sb, int typeflag,
			      struct FTW *ftwbuf)
{
	struct pcie_walk_data *walk_data = (struct pcie_walk_data *)ftwbuf;
	int ret;
	...
	if (((vid == walk_data->match_vid) || (walk_data->match_vid == 0xffff)) &&
	    ((pid == walk_data->match_pid) || (walk_data->match_pid == 0xffff))) {
		...
		ret = walk_data->cb(device_path_copy, sizeof(device_path_copy),
				    walk_data->user_data);
		if (ret < 0) {
			return -1; /* Stop walking on error */
		}
		walk_data->counter += ret;
	}
