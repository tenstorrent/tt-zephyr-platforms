...
class TTPyluwenRunner(ZephyrBinaryRunner):
    def __init__(self, cfg, pyluwen_port, asic_id):
        ...
        self.pyluwen_script = os.path.join(
            os.path.dirname(__file__), "pyluwen_gdb_remote.py"
        )
    ...
    def do_add_parser(cls, parser):
        ...
        parser.add_argument(
            "--asic-id", type=int, default=0, help="ASIC ID for pyluwen (default: 0)"
        )
    ...
    def do_run_server_and_gdb(self):
        ...
        gdb_cmd = [
            ...
            self.elf,
            "-ex",
            f"target remote localhost:{self.gdb_port}",
        ]
