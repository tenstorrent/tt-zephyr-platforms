...
/* Build with: gcc -O0 -g -Wall -Wextra -Werror -std=gnu11 -o tt-console console.c */
...
enum tlb_order {
	...
	TLB_ORDER_POSTED_RELAXED, /* Posted AXI Writes. Strict NOC ordering */
...
static void dump_vuart_desc(const struct console *cons)
{
	...
	D(2,
	  ...
	  "  rx_head: %u\n"
	  "  rx_tail: %u\n",
	  cons->vuart, vuart.magic, vuart.tx_cap, vuart.rx_cap, vuart.tx_head, vuart.tx_tail,
	  vuart.tx_oflow, vuart.rx_head, vuart.rx_tail);
}
...
static int find_vuart(struct console *cons)
{
	...
	if (vuart_magic != UART_TT_VIRT_MAGIC) {
		cons->vuart_addr = arc_read32(cons, UART_TT_VIRT_DISCOVERY_ADDR);
		D(2, "UART_TT_VIRT_DISCOVERY_ADDR: 0x%08x", cons->vuart_addr);
		...
		if (cons->vuart->magic != UART_TT_VIRT_MAGIC) {
...
static inline size_t vuart_space(struct console *cons)
{
	...
	if (vuart->magic != UART_TT_VIRT_MAGIC) {
		return 0;
	}
...
static inline void vuart_putc(struct console *cons, int ch)
{
	...
	if (tt_vuart_buf_space(vuart->rx_cap, vuart->rx_head, vuart->rx_tail) > 0) {
		++vuart->rx_tail;
	}
...
static void usage(const char *progname)
{
	I("Firmware console application for use with Tenstorrent PCIe cards\n"
	  ...
	  "-i <pci_device_id> : path to device node (default: %04x)\n"
	  "-t <tlb_id>        : 2MiB TLB index (default: %u)\n"
	  "-v                 : increase debug verbosity\n",
	  __func__, progname, TT_DEVICE, BH_2M_TLB_UC_DYNAMIC_START + 1, BH_SCRAPPY_PCI_DEVICE_ID);
}
...
static int parse_args(struct console *cons, int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, ":d:hi:t:v")) != -1) {
		switch (c) {
		case 'd':
