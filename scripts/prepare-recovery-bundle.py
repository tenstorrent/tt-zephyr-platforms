...
def generate_recovery_assets(boardname, board_data, outdir):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cmd = ['west', 'build', '-p', '-d', str(dmc_build_dir), "-b",
            f"tt_blackhole@{boardname}/tt_blackhole/dmc", "--sysbuild",
            str(app_dir / 'dmc')]
        print(f"Building for DMC on {boardname}: {' '.join(cmd)}")
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        smc_build_dir = Path(temp_dir) / 'smc_build'
        cmd = ['west', 'build', '-p', '-d', str(smc_build_dir), "-b",
            f"tt_blackhole@{boardname}/tt_blackhole/smc", "--sysbuild",
            str(app_dir / 'smc')]
        print(f"Building for SMC on {boardname}: {' '.join(cmd)}")
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        env = {"$ROOT": str(Path(__file__).parents[1]), "$BUILD_DIR": str(smc_build_dir)}

        # Now, generate bootfs hex images for every ASIC on the board
        for asic in board_data:
            bootfs = tt_boot_fs.FileImage.load(smc_build_dir / f"{asic['bootfs-name']}.yaml", env).to_boot_fs()
            ...
            if 'bmfw' in bootfs.entries:
                ...
                ih.loadfile(dmc_build_dir / 'mcuboot' / 'zephyr' / 'zephyr.hex', format='hex')
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cmd = ['cmake', f"-B{temp_dir}/build-bh-flm", '-S',
            str(Path(__file__).parent / 'tooling' / 'blackhole_recovery' / 'data' / 'bh_flm')]
        ...
        shutil.copy(
            Path(__file__).parent / 'tooling' / 'blackhole_recovery' / 'data' / 'bh_flm' / 'pyocd_config.py',
            Path(temp_dir) / 'pyocd_config.py'
        )
        # Write board manifest file
        shutil.copy(
            Path(__file__).parent / 'board_metadata.yaml',
            Path(temp_dir) / 'board_metadata.yaml'
        )
        # Drop the git revision into a text file
        git_rev = subprocess.run(['git', 'describe'], capture_output=True,
