...
for csv_path in csv_files:
    ...
    x = df["Timestamp"] if "Timestamp" in df.columns and df["Timestamp"].notnull().any() else df.index
    ...
    max_power = df["Max Power (W)"].astype(float) if "Max Power (W)" in df.columns else avg_power
    min_power = df["Min Power (W)"].astype(float) if "Min Power (W)" in df.columns else avg_power
    ...
    fig.add_trace(
        go.Scatter(
            ...
            customdata=df["Workflow Run URL"] if "Workflow Run URL" in df.columns else None,
            error_y=dict(
                type="data",
                symmetric=False,
                array=error_upper,
