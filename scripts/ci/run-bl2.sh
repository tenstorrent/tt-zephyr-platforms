...
# This script is run within CI to execute BL2 update tests.
...
if [[ "$TEST_SET" == *"bl2"* ]]; then
	# Load a DMFW that supports firmware update, so we can validate
	# the full flash flow
	$ZEPHYR_BASE/scripts/twister -i \
		...
		--west-flash \
		-T $TT_Z_P_ROOT/app \
		--outdir $ZEPHYR_BASE/twister-dmc-e2e-flash \
		$@
