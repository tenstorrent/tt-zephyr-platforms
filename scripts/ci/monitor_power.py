...
def parse_value_with_unit(value_str, unit_str):
    ...
    try:
        ...
        unit = unit_str.strip()  # preserve case for units if needed, but usually we just check suffix
...
def get_power_from_sensors():
    """
    Query board power using the sensors command from kmd driver.
    Returns power value in watts, or None if query fails.
    """
    try:
        ...
        if result.returncode == 0:
            ...
            if power_matches:
                try:
                    ...
                    power_values = [parse_value_with_unit(p[0], p[1]) for p in power_matches]
            ...
            for pattern in fallback_patterns:
                ...
                if matches:
                    try:
                        power_values = [parse_value_with_unit(p[0], p[1]) for p in matches]
            ...
            if voltage_matches and current_matches:
                try:
                    ...
                    total_voltage = sum(parse_value_with_unit(v[0], v[1]) for v in voltage_matches)
                    total_current = sum(parse_value_with_unit(c[0], c[1]) for c in current_matches)
...
def monitor_power(output_file, interval=1.0, duration=None):
    ...
    with open(output_file, "w", newline="") as csvfile:
        ...
        print(f"Starting power monitoring using sensors command. Output: {output_file}", file=sys.stderr)
            ...
            while True:
                ...
                if current_time - last_sample_time >= interval:
                    ...
                    writer.writerow({
                        ...
                        "power_watts": f"{power:.2f}" if power is not None else "N/A",
                    })
                    csvfile.flush()
