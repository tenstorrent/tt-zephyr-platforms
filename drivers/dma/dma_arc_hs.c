...
static int dma_arc_hs_status_poll(const struct device *dev, struct arc_dma_channel *chan,
				  struct dma_status *stat, k_spinlock_key_t key)
{
	struct arc_dma_data *data = dev->data;
	const struct arc_dma_config *dev_config = dev->config;
	...
	bool trigger_linked =
		(chan->config.source_chaining_en != 0U) || (chan->config.dest_chaining_en != 0U);
				  ...
				  struct dma_status *stat, k_spinlock_key_t key)
{
	...
	ARG_UNUSED(chan);
	ARG_UNUSED(stat);
	/* Should not be reached if interrupts are available */
	k_spin_unlock(&((struct arc_dma_data *)dev->data)->lock, key);
	return 0;
}
#endif
				 ...
				 struct dma_status *stat)
{
	...
	return dma_arc_hs_status_poll(dev, chan, stat, key);
}
...
static void dma_arc_hs_check_completion(const struct device *dev, uint32_t channel)
{
	...
	/* Check hardware status directly (matching original check_completion behavior) */
	uint32_t done_status = dma_arc_hs_get_done_hw(handle);
