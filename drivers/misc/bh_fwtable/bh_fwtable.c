...
const FwTable *tt_bh_fwtable_get_fw_table(const struct device *dev)
{
	...
	if (!data->initialized && !IS_ENABLED(CONFIG_TT_SMC_RECOVERY)) {
		return NULL;
	}
...
static int tt_bh_fwtable_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_TT_SMC_RECOVERY)) {
		result = tt_bh_fwtable_load(dev, BH_FWTABLE_BOARDCFG, fd_boardcfg);
	} else {
		result = (tt_bh_fwtable_load(dev, BH_FWTABLE_FLSHINFO, fd_flashinfo) ||
