...
{
	struct uart_tt_virt_data *const data = dev->data;
	k_spinlock_key_t rx_key, tx_key;

	K_SPINLOCK(&data->err_lock) {
		data->err_irq_en = false;
	}
	rx_key = k_spin_lock(&data->rx_lock);
	tx_key = k_spin_lock(&data->tx_lock);
	if (!(data->rx_irq_en || data->tx_irq_en)) {
		/* If other interrupts are disabled, stop timer */
		k_timer_stop(&data->irq_timer);
	}
	k_spin_unlock(&data->tx_lock, tx_key);
	k_spin_unlock(&data->rx_lock, rx_key);
}
...
{
	struct uart_tt_virt_data *const data = dev->data;
	k_spinlock_key_t tx_key, err_key;

	K_SPINLOCK(&data->rx_lock) {
		data->rx_irq_en = false;
	}
	tx_key = k_spin_lock(&data->tx_lock);
	err_key = k_spin_lock(&data->err_lock);
	if (!(data->tx_irq_en || data->err_irq_en)) {
		/* If other interrupts are disabled, stop timer */
		k_timer_stop(&data->irq_timer);
	}
	k_spin_unlock(&data->err_lock, err_key);
	k_spin_unlock(&data->tx_lock, tx_key);
}
...
{
	struct uart_tt_virt_data *const data = dev->data;
	k_spinlock_key_t rx_key, err_key;
