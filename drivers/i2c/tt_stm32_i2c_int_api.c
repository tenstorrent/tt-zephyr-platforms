...
int tt_stm32_i2c_send_message(const struct device *dev, uint16_t slave, struct i2c_msg msg)
{
  ...
  // decision based on a read for example SMBus BlockRead require that restart is set in order to
  // send an address + start
  bool restart = (msg.flags & I2C_MSG_RESTART) != 0;

  // To ensure that we are not expecting to reprogram address or reload settings do some validation
  // Based on the reference, because we are setting NBYTE we will always send a NACK even if we
