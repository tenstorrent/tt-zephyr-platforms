...
int tt_stm32_i2c_runtime_configure(const struct device *dev, uint32_t config)
{
  ...
  if (IS_ENABLED(STM32_I2C_DOMAIN_CLOCK_SUPPORT) && (cfg->pclk_len > 1)) {
    if (clock_control_get_rate(clk, (clock_control_subsys_t)&cfg->pclken[1], &i2c_clock) < 0) {
      LOG_ERR("Failed call clock_control_get_rate(pclken[1])");
      return -EIO;
    }
  } else {
    if (clock_control_get_rate(clk, (clock_control_subsys_t)&cfg->pclken[0], &i2c_clock) < 0) {
                               ...
                               uint8_t num_msgs)
{
  ...
  while (num_msgs > 0) {
    ...
    if (num_msgs > 1) {
      next = current + 1;
      cont = ((next->flags & I2C_MSG_RESTART) == 0) && ((current->flags & I2C_MSG_STOP) == 0) &&
...
#define STM32_I2C_IRQ_CONNECT_AND_ENABLE(index)                                                    \
  do {                                                                                             \
    IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), tt_stm32_i2c_combined_isr,      \
...
#define STM32_I2C_INIT(index)                                                                      \
  ...
  static const struct tt_stm32_i2c_config i2c_stm32_cfg_##index = {                                \
    ...
    IF_ENABLED(CONFIG_TT_I2C_STM32_SELECT_GPIOS,                                                   \
      ...
      .timings = (const struct tt_i2c_config_timing *)i2c_timings_##index,                         \
  ...
  I2C_DEVICE_DT_INST_DEFINE(index, tt_stm32_i2c_init, PM_DEVICE_DT_INST_GET(index),                \
                            &i2c_stm32_dev_data_##index, &i2c_stm32_cfg_##index, POST_KERNEL,      \
