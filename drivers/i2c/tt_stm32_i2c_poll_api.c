...
static int tt_stm32_i2c_msg_loop(const struct device *dev, struct i2c_msg *msg, bool force_reload)
{
  ...
  while (1) {
    ...
    if (!(write_waiting || read_waiting) && LL_I2C_IsActiveFlag_TC(i2c)) {
      ...
      if (len > 0) {
        LOG_ERR("Message not written before TC: {ts: %d, len: %d}", LL_I2C_GetTransferSize(i2c),
                len);
    ...
    if (!(write_waiting || read_waiting) && LL_I2C_IsActiveFlag_TCR(i2c)) {
      int sub = msg->len > STM32_I2C_MAX_TRANSFER_SIZE ? STM32_I2C_MAX_TRANSFER_SIZE : msg->len;
      if (len + sub > msg->len) {
        LOG_ERR("Message not written before reload: {ts: %d, len: %d, sub: %d}",
                              ...
                              bool start, bool force_reload)
{
  ...
  // Invalid condition we must be in reload mode if we aren't sending a start at the beginning of
  // our message.
  if (!restart && !LL_I2C_IsEnabledReloadMode(i2c)) {
    msg_abort(dev);
    return -EINVAL;
  ...
  if (restart) {
    ...
    LL_I2C_SetTransferSize(i2c, msg.len > STM32_I2C_MAX_TRANSFER_SIZE ? STM32_I2C_MAX_TRANSFER_SIZE
                                                                      : msg.len);
    LL_I2C_Enable(i2c);
