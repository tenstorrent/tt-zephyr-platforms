...
int i2c_start(struct tt_i2c_bitbang *context)
{
  ...
  if (!i2c_get_sda(context)) {
    ...
    // NOTE(drosen): I don't need to handle clock stretching here... but I'm feeling paranoid today
    if ((ret = i2c_scl_high(context))) {
      return ret;
    }
...
int i2c_write_byte(struct tt_i2c_bitbang *context, uint8_t byte)
{
  ...
  do {
    if ((ret = i2c_write_bit(context, byte & mask))) {
      return ret;
    }
  } while (mask >>= 1);
                              ...
                              bool start, bool cont)
{
  ...
  if (msg.flags & I2C_MSG_RESTART) {
    ...
    if (start) {
      ...
      if ((start_ret = i2c_scl_high(context))) {
        ret = start_ret;
        goto finish;
      }
    ...
    // NOTE(drosen): I might want to look into what bus reset condition would be appropriate for
    // this failure.
    if (start_ret) {
      ret = start_ret;
      goto finish;
    ...
    if (ack < 0) {
      ...
      // If we don't get an ack when starting it's probably safe to assume that the endpoint isn't
      // on the bus. Not getting an ACK during a restart is probably a bigger problem...
      if (!start) {
  ...
  if ((msg.flags & I2C_MSG_RW_MASK) == I2C_MSG_READ) {
    ...
    while (buf < buf_end) {
      ...
      *buf++ = (uint8_t)byte;
      // I want to support writing multiple messages back-to-back without a restart.
