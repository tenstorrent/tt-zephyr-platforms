...
{
  struct tt_smbus_stm32_data *data = CONTAINER_OF(work, struct smbus_stm32_data, smbalert_work);
                                         ...
                                         uint8_t command, uint8_t *byte)
{
  ...
  int output = i2c_write_read(config->i2c_dev, periph_addr, &command, sizeof(command), buffer, 2);
                                         ...
                                         uint8_t command, uint16_t *word)
{
  ...
  result =
    i2c_write_read(config->i2c_dev, periph_addr, &command, sizeof(command), word, sizeof(*word));
                                      ...
                                      uint8_t command, uint8_t count, uint8_t *buf)
{
  ...
  uint8_t pec = 0;
  pec = Crc8(pec, periph_addr << 1 | 0 /* I2C_WRITE_BIT */); // Address byte needs to be included
                                     ...
                                     uint8_t command, uint8_t *count, uint8_t *buf)
{
  int ret;
  uint8_t pec = 0;
  pec = Crc8(pec, periph_addr << 1 | 1 /* I2C_READ_BIT */); // Address byte needs to be included
  pec = Crc8(pec, command);
...
#define SMBUS_STM32_DEVICE_INIT(n)                                                                 \
  ...
  SMBUS_DEVICE_DT_INST_DEFINE(n, tt_smbus_stm32_init, NULL, &smbus_stm32_data_##n,                 \
                              &smbus_stm32_config_##n, POST_KERNEL, CONFIG_SMBUS_INIT_PRIORITY,    \
                              &smbus_stm32_api);
