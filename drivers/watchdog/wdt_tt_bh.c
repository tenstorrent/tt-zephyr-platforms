...
{
	Cm2BmMsg msg = {
		.msg_id = kCm2BmMsgIdAutoRstTimeoutUpdate, .data = 0 /* in ms */
	};
	EnqueueCm2BmMsg(&msg);
...
{
	Cm2BmMsg msg = {
		.msg_id = kCm2BmMsgIdAutoRstTimeoutUpdate, .data = cfg->window.min /* in ms */
	};
	EnqueueCm2BmMsg(&msg);
...
static int wdt_tt_bh_feed(const struct device *dev, int channel_id)
{
	...
	Cm2BmMsg msg = {
		.msg_id = kCm2BmMsgTelemHeartbeatUpdate, .data = data->heartbeat++, /* in ms */
	};
