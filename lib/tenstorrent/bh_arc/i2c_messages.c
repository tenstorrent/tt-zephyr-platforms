                                   ...
                                   struct response *response)
{
    ...
    if (!valid_id) return !valid_id;
    uint8_t I2C_slave_address = BYTE_GET(request->data[0], 2) & 0x7F; // Obtains the first 7 bits
    ...
    uint32_t status = I2CTransaction(I2C_mst_id, write_data_ptr, num_write_bytes, read_data_ptr, num_read_bytes);
