...
static uint32_t selected_pd_delay_chain = 0;
...
static inline void PVTInterruptConfig() {
...
static inline void PVTClkConfig() {
...
static void SdifWrite(uint32_t status_reg_addr, uint32_t wr_data_reg_addr, uint32_t sdif_addr, uint32_t data) {
...
static void EnableAgingMeas() {
  PDIpCfg0U ip_cfg0;
  ip_cfg0.val = 0;
  ip_cfg0.f.oscillator_enable = ALL_AGING_OSC;
  SdifWrite(PVT_CNTL_PD_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_PD_CMN_SDIF_REG_ADDR, IP_CFG0_ADDR, ip_cfg0.val);
}
...
void PVTInit() {
  ...
  // Configure TS
  SdifWrite(PVT_CNTL_TS_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_TS_CMN_SDIF_REG_ADDR, IP_TMR_ADDR, 0x100); // 256 cycles for TS
  SdifWrite(PVT_CNTL_TS_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_TS_CMN_SDIF_REG_ADDR, IP_CFG0_ADDR, 0x0); // use 12-bit resolution, MODE_RUN_0
  SdifWrite(PVT_CNTL_TS_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_TS_CMN_SDIF_REG_ADDR, IP_CNTL_ADDR, 0x108); // ip_run_cont

  // Configure PD
  SdifWrite(PVT_CNTL_PD_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_PD_CMN_SDIF_REG_ADDR, IP_TMR_ADDR, 0x0); // 0 cycles for PD
  SdifWrite(PVT_CNTL_PD_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_PD_CMN_SDIF_REG_ADDR, IP_CNTL_ADDR, 0x100); // ip_auto to release reset and pd
  EnableAgingMeas();

  // Configure VM
  SdifWrite(PVT_CNTL_VM_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_VM_CMN_SDIF_REG_ADDR, IP_TMR_ADDR, 0x40); // 64 cycles for VM
  SdifWrite(PVT_CNTL_VM_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_VM_CMN_SDIF_REG_ADDR, IP_CFG0_ADDR, 0x1000); // use 14-bit resolution, MODE_RUN_0, select supply check
  SdifWrite(PVT_CNTL_VM_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_VM_CMN_SDIF_REG_ADDR, IP_CNTL_ADDR, 0x108); // ip_auto to release reset and pd
...
static void SelectDelayChainAndStartPDConv(uint32_t delay_chain)
{
  if (delay_chain != selected_pd_delay_chain) {
    ...
    SdifWrite(PVT_CNTL_PD_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_PD_CMN_SDIF_REG_ADDR, IP_CFG0_ADDR, ip_cfg0.val);
    SdifWrite(PVT_CNTL_PD_CMN_SDIF_STATUS_REG_ADDR, PVT_CNTL_PD_CMN_SDIF_REG_ADDR, IP_CNTL_ADDR, 0x108); // ip_run_cont
...
static uint8_t read_ts_handler(uint32_t msg_code, const struct request * request, struct response *response)
{
...
static uint8_t read_pd_handler(uint32_t msg_code, const struct request *request, struct response *response)
{
...
static uint8_t read_vm_handler(uint32_t msg_code, const struct request * request, struct response *response)
{
  uint32_t id = request->data[1];
  uint16_t dout;
  ReadVM(id, &dout);
