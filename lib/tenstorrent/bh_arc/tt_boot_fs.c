...
tt_checksum_res_t calculate_and_compare_checksum(uint8_t *data, uint32_t num_bytes,
						 uint32_t expected, bool skip_checksum)
{
	if (!skip_checksum) {
		...
		switch (num_bytes % 4) {
		case 3:
			calculated_checksum += *data_as_dwords & 0x000000ff;
			break;
		case 2:
			calculated_checksum += *data_as_dwords & 0x0000ffff;
			break;
		case 1:
			calculated_checksum += *data_as_dwords & 0x00ffffff;
			break;
		case 0:
			calculated_checksum += *data_as_dwords & 0xffffffff;
			break;
		}
...
int load_bin_by_tag(tt_boot_fs *tt_boot_fs, const uint8_t *tag, uint8_t *dst,
		    const uint32_t dst_max_size, uint32_t *bin_size)
{
	...
	*bin_size = fd_data.flags.f.image_size;
	...
	if (calculate_and_compare_checksum(dst, fd_data.flags.f.image_size, fd_data.data_crc,
