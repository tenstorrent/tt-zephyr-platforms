...
int load_flash_info_table(uint8_t *buffer_space, uint32_t buffer_size) {
    ...
    uint32_t bin_size = 0;
    if (load_bin_by_tag(&boot_fs_data, flashInfoTableTag, buffer_space,
  ...
  bool status = pb_decode_ex(&stream, &FlashInfoTable_msg, &flash_info_table, PB_DECODE_NULLTERMINATED);
