...
bool PcieDmaWriteTransfer(uint64_t chip_addr, uint64_t host_addr, uint32_t transfer_size_bytes, 
  ...
  WriteDbiReg(HDMA_REG_ADDR(MSI_ABORT_LOW_OFF_WRCH_0), low32(msi_completion_addr + sizeof(uint32_t)));
  WriteDbiReg(HDMA_REG_ADDR(MSI_ABORT_HIGH_OFF_WRCH_0), high32(msi_completion_addr + sizeof(uint32_t)));
...
bool PcieDmaReadTransfer(uint64_t chip_addr, uint64_t host_addr, uint32_t transfer_size_bytes, 
  ...
  WriteDbiReg(HDMA_REG_ADDR(MSI_ABORT_LOW_OFF_RDCH_0), low32(msi_completion_addr + sizeof(uint32_t)));
  WriteDbiReg(HDMA_REG_ADDR(MSI_ABORT_HIGH_OFF_RDCH_0), high32(msi_completion_addr + sizeof(uint32_t)));
...
static uint8_t pcie_dma_transfer_handler(uint32_t msg_code, const struct request *request, struct response *response)
{
  ...
  if (msg_code == MSG_TYPE_PCIE_DMA_HOST_TO_CHIP_TRANSFER)
    accept = PcieDmaReadTransfer(chip_addr, host_addr, transfer_size_bytes, msi_completion_addr, completion_data);
  else
    accept = PcieDmaWriteTransfer(chip_addr, host_addr, transfer_size_bytes, msi_completion_addr, completion_data);
