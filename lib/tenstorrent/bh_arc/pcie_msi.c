...
#define PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_PCI_MSI_CAP_ID_NEXT_CTRL_REG_REG_ADDR      \
  0x00000050
#define PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_04H_REG_REG_ADDR 0x00000054
#define PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_08H_REG_REG_ADDR 0x00000058
#define PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_0CH_REG_REG_ADDR 0x0000005C
...
void SendPcieMsi(uint8_t pcie_inst, uint32_t vector_id) {
  BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_HDL_PATH_E982B20F_PCI_MSI_CAP_ID_NEXT_CTRL_REG_reg_u pci_msi_cap;
  pci_msi_cap.val = ReadDbiReg(PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_PCI_MSI_CAP_ID_NEXT_CTRL_REG_REG_ADDR);
  ...
  if (pci_msi_cap.f.pci_msi_enable && vector_id < vectors_allowed) {
    uint32_t msi_addr_lo = ReadDbiReg(PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_04H_REG_REG_ADDR);
    uint32_t msi_addr_hi = ReadDbiReg(PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_08H_REG_REG_ADDR);
    uint64_t msi_addr = ((uint64_t)msi_addr_hi << 32) | msi_addr_lo;
    uint32_t msi_data = ReadDbiReg(PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_0CH_REG_REG_ADDR);
    msi_data += vector_id;
...
static uint8_t send_pcie_msi_handler(uint32_t msg_code, const struct request *request, struct response *response) {
