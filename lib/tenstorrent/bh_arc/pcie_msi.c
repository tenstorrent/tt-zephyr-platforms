...
void SendPcieMsi(uint8_t pcie_inst, uint32_t vector_id)
{
	...
	pci_msi_cap.val = ReadDbiReg(
		PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_PCI_MSI_CAP_ID_NEXT_CTRL_REG_REG_ADDR);
	...
	if (pci_msi_cap.f.pci_msi_enable && vector_id < vectors_allowed) {
		uint32_t msi_addr_lo = ReadDbiReg(
			PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_04H_REG_REG_ADDR);
		uint32_t msi_addr_hi = ReadDbiReg(
			PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_08H_REG_REG_ADDR);
		uint64_t msi_addr = ((uint64_t)msi_addr_hi << 32) | msi_addr_lo;
		uint32_t msi_data = ReadDbiReg(
			PCIE_DBI_USP_A_BH_PCIE_DWC_PCIE_USP_PF0_MSI_CAP_MSI_CAP_OFF_0CH_REG_REG_ADDR);
		msi_data += vector_id;
