						      ...
						      struct response *rsp)
{
	...
	bool translation_was_enabled = IsNocTranslationEnabled();
	...
	sys_write32(risc_val & ~(1u << bit_index), risc_addr);

	/* Tile reset assert */
	tensix_val = sys_read32(tile_addr);
	sys_write32(tensix_val & ~(1u << bit_index), tile_addr);
	...
	TensixInitSingleTile(phys_x, phys_y);
