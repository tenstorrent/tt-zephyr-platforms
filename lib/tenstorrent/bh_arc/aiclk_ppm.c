...
void InitAiclkPPM() {
  ...
  aiclk_ppm.fmax = CLAMP(get_fw_table()->chip_limits.asic_fmax, AICLK_FMAX_MIN, AICLK_FMAX_MAX);
  aiclk_ppm.fmin = CLAMP(get_fw_table()->chip_limits.asic_fmin, AICLK_FMIN_MIN, AICLK_FMIN_MAX);
...
static uint8_t AiclkBusyHandler(uint32_t msg_code, const struct request *request, struct response *response) {
...
static uint8_t ForceAiclkHandler(uint32_t msg_code, const struct request *request, struct response *response) {
