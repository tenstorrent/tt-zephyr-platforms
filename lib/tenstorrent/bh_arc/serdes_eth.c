...
void LoadSerdesEthRegs(uint32_t serdes_inst, uint32_t ring, const SerdesRegData *reg_table, uint32_t reg_count) {
...
int LoadSerdesEthFw(uint32_t serdes_inst, uint32_t ring, uint8_t *fw_image, uint32_t fw_size) {
  ...
  volatile uint32_t *serdes_tlb = GetTlbWindowAddr(ring, SERDES_ETH_SETUP_TLB, SERDES_INST_SRAM_ADDR(serdes_inst));
