...
void VoltageArbRequest(VoltageRequestor req, uint32_t voltage) {
  voltage_arbiter.req_voltage[req] = CLAMP(voltage, voltage_arbiter.vdd_min, voltage_arbiter.vdd_max);
}

void CalculateTargVoltage() {
...
static uint8_t ForceVddHandler(uint32_t msg_code, const struct request *request, struct response *response) {
