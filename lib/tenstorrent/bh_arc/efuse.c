...
#define EFUSE_CTRL_REG_START_ADDR(box_id) (EFUSE_DFT0_CNTL_REG_MAP_BASE_ADDR + (box_id) * EFUSE_BOX_ADDR_ALIGN)
...
#define GET_EFUSE_CNTL_ADDR(box_id, reg_name) (EFUSE_##reg_name##_REG_OFFSET + EFUSE_CTRL_REG_START_ADDR(box_id))
...
uint32_t EfuseRead(EfuseAccessType acc_type, EfuseBoxId efuse_box_id, uint32_t offset) {
  if (acc_type == EfuseDirect) {
    uint32_t volatile *p_efuse = (uint32_t volatile *) EFUSE_BOX_START_ADDR(efuse_box_id);
