...
static void write_static_telemetry(uint32_t app_version)
{
	...
	telemetry[TAG_BOARD_ID_HIGH] = tt_bh_fwtable_get_read_only_table(fwtable_dev)->board_id >> 32;
	telemetry[TAG_BOARD_ID_LOW] = tt_bh_fwtable_get_read_only_table(fwtable_dev)->board_id & 0xFFFFFFFF;
	 ...
	 */
	telemetry[TAG_FLASH_BUNDLE_VERSION] = tt_bh_fwtable_get_fw_table(fwtable_dev)->fw_bundle_version;
