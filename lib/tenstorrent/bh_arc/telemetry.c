...
{ 
  ...
  if (value == FLT_MAX || value == -FLT_MAX) return 0x80000000;
  ...
  if (value < 0) ret_value = -ret_value;
  return ret_value;
}

static void write_static_telemetry()
{
  telemetry_table.version = TELEMETRY_VERSION;                                                  // v0.1.0 - Please update when changes are made
...
static void update_telemetry()
{
  ...
  telemetry[ASIC_TEMPERATURE] = ConvertFloatToTelemetry(telemetry_internal_data.asic_temperature); // ASIC temperature - reported in signed int 16.16 format
  ...
  telemetry[AICLK] = GetAICLK();                                                                // first 16 bits - MAX ASIC FREQ (Not Available yet) , lower 16 bits - current AICLK
  ...
  telemetry[ETH_LIVE_STATUS] = 0x00000000;                                                       // ETH live status lower 16 bits: heartbeat status, upper 16 bits: retrain_status - Not Available yet
...
// Zephyr timer object submits a work item to the system work queue whose thread performs the task on a periodic basis.
// See: https://docs.zephyrproject.org/latest/kernel/services/timing/timers.html#using-a-timer-expiry-function
...
void StartTelemetryTimer()
{
  ...
  // Period (time interval between all timer expirations after the first one) are both set to telem_update_interval
