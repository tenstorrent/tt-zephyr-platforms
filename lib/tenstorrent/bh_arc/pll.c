...
static const PLLSettings kPLLInitialSettings[PLL_COUNT] = {
  ...
  {.pll_cntl_1 = {.f.refdiv = 2,
                  .f.postdiv = 0,
                  .f.fbdiv = 128}, // 3200 MHz. Use VCO >= 2650 MHz: https://tenstorrent.atlassian.net/browse/SYS-777
  ...
  {.pll_cntl_1 = {.f.refdiv = 2,
                  .f.postdiv = 0,
                  .f.fbdiv = 192}, // 4800 MHz
    .pll_cntl_2 = {.f.ctrl_bus1 = 0x18}, // FOUT4PHASEEN, FOUTPOSTDIVEN bits asserted
    .pll_cntl_3 = {.f.ctrl_bus5 = 1},
    .pll_cntl_5 = {.f.postdiv0 = 5, // ARCCLK - 800 MHz
                   .f.postdiv1 = 4, // AXICLK - 960 MHz to saturate PCIE DMA BW: https://tenstorrent.atlassian.net/browse/SYS-737
...
#define GET_PLL_CNTL_ADDR(ID, REG_NAME) (PLL_0_CNTL_##REG_NAME##_REG_ADDR + PLL_CNTL_REG_OFFSET * ID)
...
static void enable_clk_counters()
{
 ...
 *
 * This function tries to find a valid set of PLL settings to hit the requested GDDRMEMCLK frequency.
 * It then updates the PLL if valid settings are found.
 *
...
void SetAICLK(uint32_t aiclk_in_mhz) {
  ...
  uint32_t target_fbdiv = (aiclk_in_mhz * 4) / REFCLK_F_MHZ; // refdiv is 2, external postdiv is 1
