...
static void WaitCmdFifoNotFull() {
  uint32_t cmd_fifo_vacant_slots = 0;
  do {
    cmd_fifo_vacant_slots = ReadReg(APB2AVSBUS_AVS_FIFOS_STATUS_REG_ADDR) & GET_AVS_FIELD_MASK(FIFOS_STATUS, CMD_FIFO_VACANT_SLOTS);
  } while (cmd_fifo_vacant_slots == 0);
}

static void WaitRxFifoNotEmpty() {
  uint32_t readback_fifo_occupied_slots = 0;
  do {
    readback_fifo_occupied_slots = ReadReg(APB2AVSBUS_AVS_FIFOS_STATUS_REG_ADDR) & GET_AVS_FIELD_MASK(FIFOS_STATUS, READBACK_FIFO_OCCUPIED_SLOTS);
...
static AVSStatus ReadRxFifo(uint16_t *response) {
  ...
  if (response != NULL) {
    if (slave_ack != AVSOk)
      ...
      *response = (readback_data & GET_AVS_FIELD_MASK(READBACK, CMD_DATA)) >> GET_AVS_FIELD_SHIFT(READBACK, CMD_DATA);
  }
...
static void SendCmd(uint16_t cmd_data, uint8_t rail_sel, uint8_t cmd_code, uint8_t cmd_grp, AVSReadWriteType r_or_w) {
  ...
  uint32_t rail_sel_pos = (rail_sel << GET_AVS_FIELD_SHIFT(CMD, RAIL_SEL)) & GET_AVS_FIELD_MASK(CMD, RAIL_SEL);
  uint32_t cmd_code_pos = (cmd_code << GET_AVS_FIELD_SHIFT(CMD, CMD_CODE)) & GET_AVS_FIELD_MASK(CMD, CMD_CODE);
  uint32_t cmd_grp_pos = (cmd_grp << GET_AVS_FIELD_SHIFT(CMD, CMD_GRP)) & GET_AVS_FIELD_MASK(CMD, CMD_GRP);
  uint32_t r_or_w_pos = r_or_w << GET_AVS_FIELD_SHIFT(CMD, R_OR_W);
  WriteReg(APB2AVSBUS_AVS_CMD_REG_ADDR, cmd_data_pos | rail_sel_pos | cmd_code_pos | cmd_grp_pos | r_or_w_pos); 
}
...
void AVSInit() {
  ...
  // gate all clocks entering AVS clock mux - do this before changeing the clock divider settings.
  avs_cfg_1.f.turn_off_all_premux_clocks = 1;
  WriteReg(APB2AVSBUS_AVS_CFG_1_REG_ADDR, avs_cfg_1.val);
  // use divided version of APB clock as AVS clock, and set the divider value to get a clock of 20MHz.
...
AVSStatus AVSReadSystemInputCurrent(uint16_t *response) {
  ...
  // The actual input current depends on how the current signal is converted to a voltage at the IINSEN pin. In the case of the MAX20816 EV Kit,
