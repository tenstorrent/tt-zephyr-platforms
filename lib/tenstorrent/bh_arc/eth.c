...
uint32_t GetEthSel(uint32_t eth_enabled)
{
	...
	} else if (tt_bh_fwtable_get_fw_table(fwtable_dev)->pci0_property_table.num_serdes == 1) { /* Only enable ETH 2,3 */
		eth_sel |= BIT(2) | BIT(3);
	}
	...
	if (tt_bh_fwtable_get_fw_table(fwtable_dev)->eth_property_table.eth_disable_mask_en) {
		eth_sel &= ~tt_bh_fwtable_get_fw_table(fwtable_dev)->eth_property_table.eth_disable_mask;
	}
