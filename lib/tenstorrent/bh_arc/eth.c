...
uint32_t GetEthSel() {
    ...
    if (get_fw_table()->pci0_property_table.pcie_mode == FwTable_PciPropertyTable_PcieMode_DISABLED) { // Enable ETH 0-3
        eth_sel |= BIT(0) | BIT(1) | BIT(2) | BIT(3);
    } else if (get_fw_table()->pci0_property_table.num_serdes == 1) { // Only enable ETH 2,3
        eth_sel |= BIT(2) | BIT(3);
    }
    if (get_fw_table()->pci1_property_table.pcie_mode == FwTable_PciPropertyTable_PcieMode_DISABLED) { // Enable ETH 10-13
        eth_sel |= BIT(10) | BIT(11) | BIT(12) | BIT(13);
    } else if (get_fw_table()->pci1_property_table.num_serdes == 1) { // Only enable ETH 10,11
    ...
    // Make sure to send the mux_sel information as well so the ETH can configure itself correctly to SerDes lanes
    // This is mainly for edge cases where a mux_sel enabled ETH is forcefilly disabled by the eth_disable_mask
    // e.g. if pcie0 mux_sel is 0b00, ETH4 goes to SerDes 3 Lane 3:0, ETH5 goes to SerDes 3 Lane 7:4
    //      but eth_disable_mask is 0b10000, then ETH4 is disabled and only ETH5 is enabled via eth_sel,
    //      at which point it becomes ambiguous which SerDes lane ETH5 should be connected to (3:0 or 7:4?)
    //      having the mux_sel information will allow ETH5 to disambiguate this
    return (pcie1_misc_cntl3_reg.f.mux_sel << 24) | (pcie_misc_cntl3_reg.f.mux_sel << 16) | eth_sel;
}

uint64_t GetMacAddressBase() {
...
void ReleaseEthReset(uint32_t eth_inst, uint32_t ring) {
    ...
    volatile uint32_t *soft_reset_0 = GetTlbWindowAddr(ring, ETH_SETUP_TLB, ETH_RISC_DEBUG_SOFT_RESET_0);
...
int LoadEthFwCfg(uint32_t eth_inst, uint32_t ring, uint8_t* fw_cfg_image, uint32_t fw_cfg_size) {
    ...
    fw_cfg_32b[0] = GetEthSel(); // Pass in eth_sel based on harvesting info and board type

    // Pass in some board/chip specific data for ETH to use
    // InitHW -> InitEth -> LoadEthFwCfg comes before init_telemtry, so cannot simply call for telemetry data here
    ...
    // Split the 48-bit MAC address into 2 24-bit values, separated by organisation ID and device ID
