...
static void HarvestingATEFuses() {
...
static uint32_t GetTensixDisableSLTMapV1() {
  ...
  if ((slt_binning == 3 || slt_binning == 4 || slt_binning == 5) &&
      harvested_count < 2) {
...
static void HarvestingSLTFuses() {
  ...
  if (slt_fuse_map_version == 1) {
    ...
    tile_enable.tensix_col_enabled &= ~READ_FUNCTIONAL_EFUSE(SLT_HARVESTED_TENSIX_COLUMNS);
  }
