...
void UartTransmitFrames(uint32_t num_frame, uint8_t *data) {
  ...
  for (uint32_t id = 0; id < num_frame; id++) {
    WriteReg(UART_ADDRESS_BLOCK_RBR_REG_ADDR, data[id]); // RBR and THR share the same address
  }
}

// receive one frame from RBR
uint8_t UartReceiveFrame() {
  WaitDataReady();
...
void UartInit() {
  ...
  WriteReg(UART_ADDRESS_BLOCK_IER_REG_ADDR, 0x1); // IER and DLH share the same register address
