...
static void enter_state0() {
  asic_state = A0State;
}

static void enter_state3() {
  ForceAiclk(800);
  ForceVdd(750);
  asic_state = A3State;
}
...
static uint8_t asic_state_handler(uint32_t msg_code, const struct request *request, struct response *response) {
