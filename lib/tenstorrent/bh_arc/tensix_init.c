 ...
 *
 * @param ring The ring to use for the multicast TLB
 * @param noc_tlb The TLB number to use for the multicast TLB
 * @param addr The address to load the firmware to
 */
static inline void setup_tensix_mcast_tlb(uint8_t ring, uint8_t noc_tlb, uint32_t addr)
{
...
static int wipe_dest(void)
{
	...
	setup_tensix_mcast_tlb(ring, noc_tlb, TRISC_WIPE_FW_LOAD_ADDR);
