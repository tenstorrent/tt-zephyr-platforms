...
static int SpiBlockRead(uint32_t spi_address, uint32_t num_bytes, uint8_t *dest)
{
	...
	if (rc < 0) {
		LOG_ERR("Flash read failed at 0x%08x: %d", spi_address, rc);
	}
	return rc;
}
...
static int SpiSmartWrite(uint32_t address, const uint8_t *data, uint32_t num_bytes)
{
	...
	if (rc < 0) {
		LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "read", 1, addr, rc);
		return rc;
	}
	if (memcmp(&spi_page_buf[address - addr], data, chunk_size) != 0) {
		...
		if (rc < 0) {
			LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "erase", 1, addr, rc);
			return rc;
		}
		rc = flash_write(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "write", 1, addr, rc);
			return rc;
		}
	}
	addr += sector_size;
	...
	while (num_bytes > sector_size) {
		...
		rc = flash_read(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "read", 2, addr, rc);
			return rc;
		}
		if (memcmp(spi_page_buf, data, MIN(sector_size, sizeof(spi_page_buf))) != 0) {
			...
			rc = flash_erase(flash, addr, sector_size);
			if (rc < 0) {
				LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "erase", 2,
					addr, rc);
				return rc;
			}
			rc = flash_write(flash, addr, data, sector_size);
			if (rc < 0) {
				LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "write", 2,
					addr, rc);
				return rc;
			}
		}
	...
	rc = flash_read(flash, addr, spi_page_buf, sector_size);
	if (rc < 0) {
		LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "read", 3, addr, rc);
		return rc;
	}
	if (memcmp(spi_page_buf, data, chunk_size) != 0) {
		...
		rc = flash_erase(flash, addr, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "erase", 3, addr, rc);
			return rc;
		}
		rc = flash_write(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash %s failed [phase %d] at 0x%08x: %d", "write", 3, addr, rc);
			return rc;
		}
	}
	return 0;
