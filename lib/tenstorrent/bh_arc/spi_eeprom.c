...
/* Declared as part of the tt_blackhole board support */
extern int spi_controller_reclock(uint32_t freq);
...
{
	if (spi_controller_reclock(MHZ(GetARCCLK())) != 0) {
		printk("Error training SPI controller\n");
	}
}
