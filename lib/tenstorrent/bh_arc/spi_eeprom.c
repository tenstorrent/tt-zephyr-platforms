...
static bool reinit_spiclk = false;
...
void SpiBufferSetup() {
  WriteReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(10), ((uint32_t)LOG2(SPI_BUFFER_SIZE) << 24) | ((uint32_t)spi_global_buffer & 0xFFFFFF));
}
...
static void PackTx(uint8_t command, uint32_t *paddress, uint8_t *data, uint32_t data_len, uint32_t *tx_data) {
  uint32_t tx_index = 0;
...
static void enter_four_byte_address_mode() {
  if (eeprom.io_mode != SpiStandardMode) {
...
static void CalibrateRxSampleDelay() {
...
void EepromSetup() {
  ...
  if (eeprom.io_mode == SpiQuadMode) {
    if (eeprom.ddr) {
      ...
      // These dummy cycles do not match with MT25 spec, needed only for QUAD DDR as the first 4b read was incorrect.
...
static void AlignProgramAddrData(uint32_t *paddress, const uint8_t *data, uint32_t *num_frames, uint8_t *adjusted_data) {
...
static inline uint32_t NextChunkSize(const uint32_t start_addr, const uint32_t offset, const uint32_t num_bytes, const uint32_t size) {
...
static bool NeedErase(const uint8_t *new_data, const uint8_t *old_data, uint32_t size) {
  // Program can only clear bits, so if new_data has a bit set that is not in old_data, we must erase
