...
static int SpiSmartWrite(uint32_t address, const uint8_t *data, uint32_t num_bytes)
{
	...
	if (rc < 0) {
		LOG_ERR("Flash read failed [phase 1] at 0x%08x: %d", addr, rc);
		return rc;
	}
	if (memcmp(&spi_page_buf[address - addr], data, chunk_size) != 0) {
		...
		if (rc < 0) {
			LOG_ERR("Flash erase failed [phase 1] at 0x%08x: %d", addr, rc);
			return rc;
		}
		rc = flash_write(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash write failed [phase 1] at 0x%08x: %d", addr, rc);
			return rc;
		}
	}
	addr += sector_size;
	...
	while (num_bytes > sector_size) {
		...
		rc = flash_read(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash read failed [phase 2] at 0x%08x: %d", addr, rc);
			return rc;
		}
		if (memcmp(spi_page_buf, data, MIN(sector_size, sizeof(spi_page_buf))) != 0) {
			...
			rc = flash_erase(flash, addr, sector_size);
			if (rc < 0) {
				LOG_ERR("Flash erase failed [phase 2] at 0x%08x: %d", addr, rc);
				return rc;
			}
			rc = flash_write(flash, addr, data, sector_size);
			if (rc < 0) {
				LOG_ERR("Flash write failed [phase 2] at 0x%08x: %d", addr, rc);
				return rc;
			}
		}
		addr += sector_size;
	...
	rc = flash_read(flash, addr, spi_page_buf, sector_size);
	if (rc < 0) {
		LOG_ERR("Flash read failed [phase 3] at 0x%08x: %d", addr, rc);
		return rc;
	}
	if (memcmp(spi_page_buf, data, chunk_size) != 0) {
		...
		rc = flash_erase(flash, addr, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash erase failed [phase 3] at 0x%08x: %d", addr, rc);
			return rc;
		}
		rc = flash_write(flash, addr, spi_page_buf, sector_size);
		if (rc < 0) {
			LOG_ERR("Flash write failed [phase 3] at 0x%08x: %d", addr, rc);
			return rc;
		}
	}
	return 0;
