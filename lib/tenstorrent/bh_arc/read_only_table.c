...
int load_read_only_table(uint8_t *buffer_space, uint32_t buffer_size) {
    uint32_t bin_size = 0;
    static const char readOnlyTag[IMAGE_TAG_SIZE] = "boardcfg";
    if (load_bin_by_tag(&boot_fs_data, readOnlyTag, buffer_space,
  ...
  bool status = pb_decode_ex(&stream, &ReadOnly_msg, &read_only_table, PB_DECODE_NULLTERMINATED);
