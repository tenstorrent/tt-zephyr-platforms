...
static inline uint32_t volatile * GetTlbRegStartAddr(const uint8_t ring) {
  uint32_t volatile *tlb_addr = (uint32_t volatile *) ((NIU_0_A_REG_MAP_BASE_ADDR + RING0_TLB_REG_OFFSET) | ((uint32_t)ring << AXI2NOC_RING_SEL_BIT));
  return tlb_addr;
}

static inline void WriteTlbSetup(const uint8_t ring, const uint8_t tlb_num, NOC2AXITlb0RegU tlb0, NOC2AXITlb1RegU tlb1, NOC2AXITlb2RegU tlb2, NOC2AXITlb3RegU tlb3) {
...
// Broadcast to all unharvested Tensix. Requires NocInit to be called first to set up broadcast disables.
