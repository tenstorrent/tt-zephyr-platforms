...
float GetVcoreCurrent() {
  ...
  uint16_t iout;
  I2CReadBytes(PMBUS_MST_ID, READ_IOUT, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &iout, READ_IOUT_DATA_BYTE_SIZE, PMBUS_FLIP_BYTES);
...
float GetVcorePower() {
  I2CInit(I2CMst, P0V8_VCORE_ADDR, I2CFastMode, PMBUS_MST_ID);
  uint16_t pout;
  I2CReadBytes(PMBUS_MST_ID, READ_POUT, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &pout, READ_POUT_DATA_BYTE_SIZE, PMBUS_FLIP_BYTES);
...
float GetVoltage(uint32_t slave_addr) {
  ...
  uint16_t vout_cmd = 0;
  I2CReadBytes(PMBUS_MST_ID, READ_VOUT, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &vout_cmd, READ_VOUT_DATA_BYTE_SIZE, PMBUS_FLIP_BYTES);
...
void I2CSetVoltage(uint32_t slave_addr, float voltage_in_mv) {
  ...
  I2CWriteBytes(PMBUS_MST_ID, VOUT_COMMAND, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &vout_cmd, VOUT_COMMAND_DATA_BYTE_SIZE);

  // 100us to flush the tx of i2c + 150us to cover voltage switch from 0.65V to 0.95V with 50us of margin
  WaitUs(250);
}
...
void SwitchVoutControl(VoltageCmdSource source) {
  ...
  I2CReadBytes(PMBUS_MST_ID, OPERATION, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &operation, OPERATION_DATA_BYTE_SIZE, PMBUS_FLIP_BYTES);
  operation.transition_control = 1; // copy vout command when control is passed from AVSBus to PMBus
  operation.voltage_command_source = source;
  I2CWriteBytes(PMBUS_MST_ID, OPERATION, PMBUS_CMD_BYTE_SIZE, (uint8_t *) &operation, OPERATION_DATA_BYTE_SIZE);
...
static uint8_t set_voltage_handler(uint32_t msg_code, const struct request *request, struct response *response)
{
...
static uint8_t get_voltage_handler(uint32_t msg_code, const struct request *request, struct response *response)
{
  uint32_t slave_addr = request->data[1];
...
static uint8_t switch_vout_control_handler(uint32_t msg_code, const struct request * request, struct response * response)
{
