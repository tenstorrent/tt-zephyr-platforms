...
static int I2CWriteHandler(struct i2c_target_config *config, uint8_t val) {
  ...
  if (smbus_data.state == kSmbusStateIdle) {
    ...
    // Calculate the PEC
    uint8_t pec = 0;
    pec = Crc8(pec, I2C_TARGET_ADDR << 1 | I2C_WRITE_BIT); // Address byte needs to be included
    ...
    if (pec != rcv_pec) {
      ...
      int32_t ret = curr_cmd->handler.rcv_handler(smbus_data.received_data, smbus_data.blocksize);
      smbus_data.state = kSmbusStateWaitIdle;
      return ret;
    }
  } else {
    WriteReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19), 0xc2de0000 | ReadReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19)));
    smbus_data.state = kSmbusStateWaitIdle;
    return -1;
  }
  return 0;
...
static int I2CReadHandler(struct i2c_target_config *config, uint8_t *val) {
  ...
  if (smbus_data.state == kSmbusStateCmd) {
    ...
    *val = smbus_data.send_data[smbus_data.send_index++];
    ...
    uint8_t pec = 0;
    pec = Crc8(pec, I2C_TARGET_ADDR << 1 | I2C_READ_BIT); // Address byte needs to be included
    pec = Crc8(pec, smbus_data.command);
    ...
    *val = pec;
    ...
    WriteReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19), 0xc1de0000 | ReadReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19)));
    smbus_data.state = kSmbusStateWaitIdle;
    return -1;
  }
  return 0;
...
static int I2CStopHandler(struct i2c_target_config *config) {
  ...
  WriteReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19), 0xc3de0000 | ReadReg(RESET_UNIT_SCRATCH_RAM_REG_ADDR(19)));
