...
{
  LOG_DBG("%s%s{spi_addr: %x, copy_dest: %x, flags: { image_size: %zu, executable: %d, invalid: "
          "%d}, data_crc: "
...
int tt_fwupdate(const char *tag, bool dry_run, bool reboot)
{
  ...
  /*
   * Alpha firmware had no means of getting signaled to initiate a firmware update from the host.
   * In that scenario, the only means of updating is to overwrite if the new image is different.
   */
#ifdef CONFIG_TT_FWUPDATE_TEST
  cksum = tt_boot_fs_cksum(0, (const uint8_t *)fake_image, sizeof(fake_image));
#else
  cksum = tt_boot_fs_cksum(0,
  ...
  if (cksum == fd.data_crc) {
    /*
     * Also do not write an update to slot1 if the existing slot1 image is identical to the update.
     */
...
int tt_fwupdate_flash_image(const tt_boot_fs_fd *fd)
{
  ...
  rc = flash_erase(flash0_dev, DT_REG_ADDR(DT_NODELABEL(slot1_partition)), erase_size) ||
       flash_copy(flash1_dev, fd->spi_addr, flash0_dev, DT_REG_ADDR(DT_NODELABEL(slot1_partition)),
                  fd->flags.f.image_size, write_buf, sizeof(write_buf));
  if (rc < 0) {
...
int tt_fwupdate_validate_image(const tt_boot_fs_fd *fd)
{
  ...
  if (fd->flags.f.image_size > DT_REG_SIZE(DT_NODELABEL(slot1_partition))) {
    LOG_ERR("image size %zu is too large for slot1-partition size %zu", fd->flags.f.image_size,
            DT_REG_SIZE(DT_NODELABEL(slot1_partition)));
    return -ENOENT;
  }

  /* Ensure that IMAGE_MAGIC is found in the first 4 bytes of the image, otherwise it will not be
   * bootable */
