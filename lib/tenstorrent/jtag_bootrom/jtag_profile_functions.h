...
#ifndef BH_BMFW_APP_JTAG_UGLY_SRC_JTAG_PROFILE_FUNCTIONS_H_
#define BH_BMFW_APP_JTAG_UGLY_SRC_JTAG_PROFILE_FUNCTIONS_H_
...
#define CYCLES_EXIT()                                                                              \
  do {                                                                                             \
    if (!__cnt.func) {                                                                             \
      ...
      printk("%s(): %d: finished in %u ms (%u cycles), %u io_ops\n", __func__, __LINE__,           \
             __cnt.cycles / (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC / 1000), __cnt.cycles,             \
             __cnt.io_ops);                                                                        \
    }                                                                                              \
  } while (0)
...
#endif /* BH_BMFW_APP_JTAG_UGLY_SRC_JTAG_PROFILE_FUNCTIONS_H_ */
